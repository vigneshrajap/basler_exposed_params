...
void handle_basler_boolean_parameter(CInstantCamera& camera, string name,
                                     bool value) {
  ...
  try {
    ...
    if (!IsWritable(this_node)) {
      ROS_ERROR_STREAM("Basler parameter '"
                       << name << "' isn't writable or doesn't exist.");
      return;
    }
...
void handle_basler_int_parameter(CInstantCamera& camera, string name,
                                 int value) {
  INodeMap& nodemap = camera.GetNodeMap();
  try {
...
void load_pfs_file(CInstantCamera& camera, string name) {
  INodeMap* nodemap = &camera.GetNodeMap();
  Pylon::String_t config_file = name.c_str();
  try {
    Pylon::CFeaturePersistence::Load(config_file, nodemap, true);
  } catch (const GenericException& e) {
    ROS_ERROR_STREAM(e.GetDescription());
  }
}

void handle_basler_float_parameter(CInstantCamera& camera, string name,
                                   double value) {
  INodeMap& nodemap = camera.GetNodeMap();
  try {
...
void handle_basler_enum_parameter(CInstantCamera& camera, string name,
                                  string value) {
  INodeMap& nodemap = camera.GetNodeMap();
  try {
    ...
    if (!IsAvailable(this_node->GetEntryByName(value.c_str()))) {
      ROS_ERROR_STREAM("Valuer '" << value
                                  << "' isn't available for basler param '"
                                  << name << "'.");
      return;
    }
...
void handle_basler_parameter(CInstantCamera& camera,
                             XmlRpc::XmlRpcValue& param) {
  ros::NodeHandle nh("~");
  string type = param["type"];
  if ("boolean" == type) {
    ROS_ASSERT_MSG(param["value"].getType() == XmlRpc::XmlRpcValue::TypeBoolean,
                   "Type of value for %s must be boolean",
    ...
    ROS_ASSERT_MSG(param["value"].getType() == XmlRpc::XmlRpcValue::TypeInt,
                   "Type of value for %s must be int",
                   string(param["name"]).c_str());
    ...
    ROS_ASSERT_MSG(param["value"].getType() == XmlRpc::XmlRpcValue::TypeDouble,
                   "Type of value for %s must be float",
                   string(param["name"]).c_str());
...
void handle_basler_parameters(CInstantCamera& camera) {
  ...
  ROS_ASSERT_MSG(params.getType() == XmlRpc::XmlRpcValue::TypeArray,
                 "Badly formed basler param yaml");
  for (size_t index = 0; index < params.size(); ++index) {
    ROS_ASSERT_MSG(params[index].getType() == XmlRpc::XmlRpcValue::TypeStruct,
                   "Badly formed basler param yaml");
...
void handle_parameters(CInstantCamera& camera) {
  ...
  if (!private_handle.getParamNames(keys)) {
    ROS_INFO(
        "No params found on parameter server, using defaults from "
        "dynamic_reconfigure.");
    return;
  }
  for (std::vector<std::string>::iterator k = keys.begin(); k != keys.end();
       ++k) {
    if ((*k).size() < prefix.size()) {
      continue;
    }
    std::pair<std::string::iterator, std::string::iterator> different =
        std::mismatch(prefix.begin(), prefix.end(), (*k).begin());
    ...
    if (param_name == "basler_params") {
      ...
      break;
               ...
               (param_name == "frame_id") || (param_name == "frame_rate") ||
...
void configure_callback(basler_camera::CameraConfig& config, uint32_t level) {
  for (std::vector<basler_camera::CameraConfig::
                       AbstractParamDescriptionConstPtr>::const_iterator _i =
           config.__getParamDescriptions__().begin();
       _i != config.__getParamDescriptions__().end(); ++_i) {
    boost::any val;
    (*_i)->getValue(config, val);
    if ("bool" == (*_i)->type) {
      handle_basler_boolean_parameter(camera, (*_i)->name,
                                      boost::any_cast<bool>(val));
    } else if ("double" == (*_i)->type) {
      handle_basler_float_parameter(camera, (*_i)->name,
                                    boost::any_cast<double>(val));
    } else if ("int" == (*_i)->type) {
      handle_basler_int_parameter(camera, (*_i)->name,
                                  boost::any_cast<int>(val));
    } else if ("str" == (*_i)->type) {
      handle_basler_enum_parameter(camera, (*_i)->name,
                                   boost::any_cast<std::string>(val));
    } else {
      ROS_FATAL_STREAM("Unknown param type for config parameter "
                       << (*_i)->name << ": " << (*_i)->type);
    }
  }
}
...
int main(int argc, char* argv[]) {
  ...
  if (!nh.getParam("camera_info_url", camera_info_url)) camera_info_url = "";

  string frame_id;
  if (!nh.getParam("frame_id", frame_id)) frame_id = "";

  std::string serial_number;
  if (!nh.getParam("serial_number", serial_number)) serial_number = "";

  std::string pfs_file;
  bool load_from_pfs = false;
  nh.getParam("pfs_file", pfs_file);
  if (!pfs_file.empty()) load_from_pfs = true;
  ...
  // Automatically call PylonInitialize and PylonTerminate to ensure the pylon
  // runtime system
  ...
  try {
    ...
    camera.RegisterImageEventHandler(new ImagePublisher(nh, cinfo, frame_id),
                                     RegistrationMode_Append, Cleanup_Delete);
    camera.RegisterConfiguration(new CAcquireContinuousConfiguration,
                                 RegistrationMode_ReplaceAll, Cleanup_Delete);

    camera.Open();

    if (!load_from_pfs) {
      ROS_WARN("Loading from yaml file");
      handle_parameters(camera);
    } else {
      ROS_WARN("Loading from pfs file %s", pfs_file.c_str());
      load_pfs_file(camera, pfs_file);
    }
    ...
    ROS_ERROR_STREAM(
        "An exception occurred during setup: " << e.GetDescription());
    exitCode = 1;
    return exitCode;
  }
  ...
  while (ros::ok()) {
    try {
      ...
      ROS_ERROR_STREAM("An exception occurred trying to start grabbing: "
                       << e.GetDescription());
      return 2;
    }
    ...
    while (camera.IsGrabbing() && ros::ok()) {
      ...
      try {
        ...
        ROS_ERROR_STREAM(
            "An exception occurred during operation: " << e.GetDescription());
        break;
      }
    }
  }
